...
/**
 * Contains Intent constants necessary for interacting with the Locale Developer
 * Platform.
 */
public final class Intent {
	 ...
	 * <p>
	 * If a condition returns UNKNOWN, then Locale will use the last known
	 * return value on a best-effort basis. Best-effort means that Locale may
	 ...
	 * Locale process. If there is no last known return value, then unknown is
	 * treated as not satisfied (false).
	 * <p>
	 * The purpose of an UNKNOWN result is to allow a plug-in condition more
	 * than 10 seconds to process a requery. A {@code BroadcastReceiver} must
	 ...
	 * {@link #RESULT_CONDITION_UNKNOWN}, subsequently request a requery, and
	...
	/**
	 * {@code Intent} action {@code String} broadcast by Locale to create or
	 * edit a plug-in setting. When Locale broadcasts this {@code Intent}, it
	 * will be sent directly to the package and class of the plug-in's
	 * {@code Activity}. The {@code Intent} may contain a {@link #EXTRA_BUNDLE}
	 ...
	 * {@link #ACTION_FIRE_SETTING} receiver. In this scenario, it is the
	 * responsibility of the Activities to store enough data in
	...
	/**
	 * {@code Intent} action {@code String} broadcast by Locale to fire a
	 * plug-in setting. When Locale broadcasts this {@code Intent}, it will be
	 * sent directly to the package and class of the plug-in's
	 * {@code BroadcastReceiver}. The {@code Intent} will contain a
	 * {@link #EXTRA_BUNDLE} that was previously set by the {@code Activity}
	 * result of {@link #ACTION_EDIT_SETTING}.
	 * <p>
	...
	/**
	 * {@code Intent} action {@code String} broadcast by Locale to create or
	 * edit a plug-in condition. When Locale broadcasts this {@code Intent}, it
	 * will be sent directly to the package and class of the plug-in's
	 * {@code Activity}. The {@code Intent} may contain a store-and-forward
	 * {@link #EXTRA_BUNDLE} that was previously set by the {@code Activity}
	 ...
	 * MAY implement multiple Activity instances that implement this
	 * {@code Intent}, however there must only be a single
	 * {@link #ACTION_QUERY_CONDITION} receiver. In this scenario, it is the
	 * responsibility of the Activities to store enough data in
	 * {@link #EXTRA_BUNDLE} to allow this receiver to disambiguate which
	...
	/**
	 * Ordered {@code Intent} action {@code String} broadcast by Locale to query
	 * a plug-in condition. When Locale broadcasts this {@code Intent}, it will
	 * be sent directly to the package and class of the plug-in's
	 * {@code BroadcastReceiver}. The {@code Intent} will contain a
	 * {@link #EXTRA_BUNDLE} that was previously set by the {@code Activity}
	 * result of {@link #ACTION_EDIT_CONDITION}.
	 * <p>
	 * Since this is an ordered broadcast, the receiver is expected to set an
	 * appropriate result code from {@link #RESULT_CONDITION_SATISFIED},
	 * {@link #RESULT_CONDITION_UNSATISFIED}, and
	...
	/**
	 * {@code Intent} action {@code String} to notify Locale that a plug-in
	 * condition is requesting that Locale query it via
	 * {@link #ACTION_QUERY_CONDITION}. This merely serves as a hint to Locale
	 * that a condition wants to be queried. There is no guarantee as to when or
	 * if the plug-in will be queried after this {@code Intent} is broadcast. If
	 * Locale does not respond to the plug-in condition after a
	 * {@link #ACTION_REQUEST_QUERY} Intent is sent, the plug-in SHOULD shut
	 * itself down and stop requesting requeries. A lack of response from Locale
	 * indicates that Locale is not currently interested in this plug-in. When
	 * Locale becomes interested in the plug-in again, Locale will send
	 * {@link #ACTION_QUERY_CONDITION}.
	 * <p>
	 * The extra {@link #EXTRA_ACTIVITY} MUST be included, otherwise Locale will
	 ...
	 * {@code Activity} result extra from {@link #ACTION_EDIT_CONDITION} or
	 * {@link #ACTION_EDIT_SETTING}.
	 * <p>
	 ...
	 * , because {@code Parcelable} is not a long-term storage format. Also,
	 * plug-ins MUST NOT store any serializable object that is not exposed by
	 * the Android SDK.
	 * <p>
	 * The maximum size of a Bundle that can be sent across process boundaries
	 * is on the order of 500 kilobytes (base-10), while Locale further limits
	 * plug-in Bundles to about 100 kilobytes (base-10). Although the maximum
	 * size is about 100 kilobytes, plug-ins SHOULD keep Bundles much smaller
	 ...
	 * Type: {@code String}.
	 * <p>
	 * Maps to a {@code String} that represents the name of a plug-in's
	 * {@code Activity}.
	 * 
	...
	public final static String VARIABLE_REPLACE_KEYS = "net.dinglisch.android.tasker.extras.VARIABLE_REPLACE_KEYS";
}